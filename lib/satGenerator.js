...
function getOrbitFeatures(sat, features, samplesStep, samplesTotal) {

    // Generate the orbit;
    var track = getOrbitTrack(sat,samplesStep,samplesTotal);
...
function addOrbitToTangramSource(sourceName, sat, samplesStep, samplesTotal) {
    var features = [];
    if (Array.isArray(sat)){
        for (s of sat){
            getOrbitFeatures(s, features, samplesStep, samplesTotal);
        }
    } else {
        getOrbitFeatures(sat, features, samplesStep, samplesTotal);
    }
